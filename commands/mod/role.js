  ...
  config : {
    ...
    run: async (client, message, args) => {
        ...
        if (message.guild.me.roles.highest.id === role.id) {
            return message.channel.send(`<:896365442925482025:914642294752301057> | **I cannot add or remove that role because that is my highest role**`)}
        ...
        if (user.roles.cache.has(role.id)) {
           ...
           const embed = new MessageEmbed()
          .setDescription(`<:896365430841679882:914642292441231360> Removed roles for ${user.user.tag}, ${role.name}`)
               .setColor('2f3136')
            return message.channel.send(embed)
           ...
           const embed = new MessageEmbed()
          .setDescription(`<:896365430841679882:914642292441231360>  Added roles for ${user.user.tag}, +${role.name}`)
               .setColor('2f3136')
            return message.channel.send(embed)
